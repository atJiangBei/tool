...
<div class="create-container">
	<div class="do-container">
		...
		<div>
			<div class="layer-size-container">
				<label>高:</label>
				<input type="number" value="1080" min="1080" max="3000" id="canvasHeight" placeholder="高度"/>
				<label>宽:</label>
				<input type="number" value="720" min="720" max="2000" id="canvasWidth" placeholder="宽度"/>
			</div>
			<div class="layer-rgb-container" id="layerRgbContainer">
				<div>
					<label>R:</label>
					<input type="number" value="240" min="0" max="255" placeholder="高度"/>
				</div>
				<label>G:</label>
				<input type="number" value="240" min="0" max="255" placeholder="宽度"/>
				<label>B:</label>
				<input type="number" value="240" min="0" max="255" placeholder="宽度"/>
				<div class="preview-block-container">
					<div class="create-img-btn" data-type="preview">预览</div>
					<div class="block-rgb-container" id="blockRgbContainer" style="background: rgb(240,240,240);"></div>
				</div>
			</div>
		...
		<div class="add-img-to-canvas" id="addImgToCanvas">
			<input type="file" id="uploadControl" accept="image/png,image/jpeg,image/jpg"/>
		</div>
		<h3>图层位置调整度</h3>
		...
		<h3>当前图层</h3>
		<div class="current-layer-container" id="currentLayerContainer">
	...
	<div class="canvas-simulation" id="canvasSimulation">
...
<script>
		function createImgByCanvas({
			...
			while(imgData.length){
				...
				ctx.drawImage(item.el,0,0,item.width,item.height,item.left,item.top,item.width,item.height)
...
export default{
	mounted(){
		...
		defineReactive(currentData,'varRemovableLayer',null,function(obj){
			console.log(obj)
			if(obj){
				currentLayerContainer.innerHTML = createHtmlFragment(obj.zIndex,obj.imgName,obj.isPeople)
			}else{
		...
		createCanvas.addEventListener("click",function(){
			if(!canCreate){
				alert("您已生成画布，次过程中不允许中途再次操做，如需重新生成，请重置画布");
				return;
			}
		...
		uploadControl.addEventListener("change",function(e){
			...
			fileToBase64(file).then(base64=>{
				...
				const onloadfn = function(){
					...
					const removableLayer = createRemovableLayer(Img,this.width,this.height);
		...
		const currentLayerContainer = document.getElementById("currentLayerContainer");
...
.add-img-to-canvas{
	width: 36px;
	height: 36px;
	background: url(http://h5.jiandantianqi.com/event/common/icon/camera.png) no-repeat center;
...
.threshold-container .threshold-add{
	...
	width: 30px;
	height: 30px;
	background: url(http://h5.jiandantianqi.com/event/common/icon/add_value.png) no-repeat center;
	background-size: 100% 100%;
}
.threshold-container .threshold-reduce{
	...
	width: 30px;
	height: 30px;
	background: url(http://h5.jiandantianqi.com/event/common/icon/reduce_value.png) no-repeat center;
	background-size: 100% 100%;
}
/* 设置画布尺寸 */
...
.set-size-container:hover{
	...
	box-shadow:0 1px 2px -2px rgba(0,0,0,.3),0 3px 6px 0 rgba(0,0,0,.3),0 5px 12px 4px rgba(0,0,0,.3);
	transition: all .3s;
}
.set-size-head{
	height: 33px;
...
.set-size-head span{
	...
	width: 28px;
	height: 25px;
	background:#FBAE00 url(http://h5.jiandantianqi.com/event/common/icon/delete.png) no-repeat center;
	background-size: 100% 100%;
}
.set-layer-name{
	font-size: 12px;
...
.set-size-content .set-size-add{
	background:#FBAE00 url(http://h5.jiandantianqi.com/event/common/icon/prev_zindex.png) no-repeat center;
	background-size: 100% 100%;
}
...
.set-size-content .set-size-reduce{
	background:#FBAE00 url(http://h5.jiandantianqi.com/event/common/icon/next_zindex.png) no-repeat center;
	background-size: 100% 100%;
}
...
.create-img-block-popup span{
	...
	width: 36px;
	height: 36px;
	background:url(http://h5.jiandantianqi.com/event/common/icon/close.png) no-repeat center;
	background-size: 100% 100%;
	cursor: pointer;
}
</style>
